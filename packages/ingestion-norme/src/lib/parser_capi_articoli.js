...
async function parseCapiArticoli(opzioni) {
    try {
        ...
        let testoCompleto = await fs.readFile(opzioni.sourceFile, 'utf-8');
...
async function parseConCapi(testoCompleto, opzioni) {
    ...
    const documento = {
        ...
        const regexCapo = /^CAPO\s+([IVXLCDM]+)(?:\s*[-–—]\s*(.*))?$/gmi;
        const regexArticolo = /^Articolo\s+(\d+)(?:\s*[-–—]\s*(.*))?$/gmi;
        
        // Normalizza i line endings
        const testoNormalizzato = testoCompleto.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
        
        // Debug: verifica la presenza dei CAPO (regex più inclusiva)
        const testCapo = testoNormalizzato.match(/CAPO\s+[IVXLCDM]+/gi);
        console.log('🔍 CAPO trovati nel testo:', testCapo);
        
        // Test specifico per IX
        const testIX = testoNormalizzato.includes('CAPO IX');
        console.log('🔍 Test CAPO IX presente:', testIX);
        
        // Dividi in righe
        const righe = testoNormalizzato.split('\n');
        
        // Approccio alternativo: trova tutte le posizioni dei CAPO con regex più ampia
        const indiciCapo = [];
        
        righe.forEach((riga, indice) => {
            // Regex più inclusiva per catturare tutti i possibili numeri romani
            if (/^\s*CAPO\s+[IVXLCDM]+\s*$/i.test(riga.trim())) {
                indiciCapo.push(indice);
                console.log(`📍 Trovato CAPO alla riga ${indice + 1}: "${riga.trim()}"`);
            }
        });
        
        if (indiciCapo.length === 0) {
            throw new Error('Nessun CAPO trovato nel testo');
        }
        ...
        for (const sezione of sezioniValide) {
            ...
            // Cerca l'intestazione del CAPO (prima riga non vuota)
            if (righe.length === 0) continue;
            
            const matchCapo = righe[0].match(/^CAPO\s+([IVXLCDM]+)\s*$/i);
            
            if (matchCapo) {
                const numeroRomano = matchCapo[1];
                const numeroArabo = romanoToArabo(numeroRomano);
                
                // Il titolo del CAPO è sulla riga successiva (se presente e non è un articolo)
                let titoloCapo = '';
                let indiceInizioArticoli = 1;
                
                if (righe.length > 1 && !righe[1].match(/^Articolo\s+\d+$/i)) {
                    titoloCapo = righe[1].trim();
                    indiceInizioArticoli = 2;
                }
                
                console.log(`📚 Processando CAPO ${numeroRomano}: "${titoloCapo}"`);
                ...
                while (i < righe.length) {
                    const riga = righe[i];
                    const matchArticolo = riga.match(/^Articolo\s+(\d+)$/i);
                    
                    if (matchArticolo) {
                        const numeroArticolo = parseInt(matchArticolo[1]);
                        let titoloArticolo = '';
                        let contenutoArticolo = '';
                        
                        // Il titolo dell'articolo è sulla riga successiva (se non è un altro articolo)
                        i++; // passa alla riga successiva
                        if (i < righe.length && !righe[i].match(/^Articolo\s+\d+$/i)) {
                            titoloArticolo = righe[i].trim();
                            i++; // passa alla riga successiva
                        }
                        
                        // Raccogli il contenuto fino al prossimo articolo o fine sezione
                        const contenutoRighe = [];
                        while (i < righe.length && !righe[i].match(/^Articolo\s+\d+$/i)) {
                            contenutoRighe.push(righe[i]);
                            i++;
                        }
                        
                        contenutoArticolo = contenutoRighe.join('\n').trim();
                        
                        console.log(`   📋 Articolo ${numeroArticolo}: "${titoloArticolo}"`);
                        
                        const articolo = {
                            numero: numeroArticolo,
                ...
                console.log(`✅ CAPO ${numeroRomano} completato: ${capo.articoli.length} articoli`);
            }
        }
...
async function parseSoloArticoli(testoNormalizzato, opzioni) {
    ...
    while (i < righe.length) {
        const riga = righe[i];
        const matchArticolo = riga.match(/^Articolo\s+(\d+)\s*$/i);
        ...
        if (matchArticolo) {
            ...
            const articolo = {
                ...
                titolo: titoloArticolo,
                contenuto: contenutoArticolo,
                paragrafi: parseContenutoArticolo(contenutoArticolo) // Analizza paragrafi, lettere, punti
            };
 ...
 */
function parseContenutoArticolo(contenuto) {

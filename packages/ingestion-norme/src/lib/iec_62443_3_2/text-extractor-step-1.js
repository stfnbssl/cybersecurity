...
function filterLines(lines) {
  ...
  for (var i=0; i<lines.length; i++) {
    ...
    if (skip > 0) {
        ...
        if (lines[i].indexOf("BS EN IEC 62443‑3‑2:2020") > -1) {}
        else if (lines[i].indexOf("IEC 62443-3-2:2020 © IEC 2020")> -1) {}
        else if (/^\s*–\s*\d+\s*–\s*$/.test(lines[i])) {}
...
function step1(params) {
  ...
  if (!inputTextPath || !outputJSONPath) {
    exitWithError('Parametri mancanti: servono "inputTextPath" e "outputJSONPath" dentro iec_62443_3_2.');
  }
...
function main() {
  ...
  switch (stepNumber) {
    case 1:
      params = config.iec_62443_3_2.step1;
      step1(params);
      break;
    default:

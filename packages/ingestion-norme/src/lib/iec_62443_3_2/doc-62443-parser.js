...
function compactRequirementsAndRationales(structure) {
  const result = [];
  ...
  for (let i = 0; i < structure.length; i++) {
    ...
    // Cerca le sezioni successive che potrebbero essere Requirement o Rationale
    for (let j = i + 1; j < structure.length; j++) {
      const nextSection = structure[j];
      
      // Se il livello successivo non è un sotto-livello di quello corrente, fermiamo la ricerca
      if (!nextSection.level.startsWith(section.level + '.')) {
        break;
      }

      // Controlla se è un Requirement
      if (nextSection.title.toLowerCase().startsWith('requirement')) {
        compactedSection.requirement = {
          lines: nextSection.lines,
          text: nextSection.lines.join(' ').trim()
        };
        processedIndices.add(j);
      }
      
      // Controlla se è un Rationale
      if (nextSection.title.toLowerCase().startsWith('rationale')) {
        compactedSection.rationale = {
          lines: nextSection.lines,
          text: nextSection.lines.join(' ').trim()
        };
        processedIndices.add(j);
    ...
    result.push(compactedSection);
  }

  return result;
}

// Funzione combinata che estrae e compatta
function extractAndCompactStructure(inputData) {
  const lines = inputData.lines || inputData;
  const parsed = parseDocument(lines);
  const formatted = formatResult(parsed);
  const compacted = compactRequirementsAndRationales(formatted.structure);
...
module.exports = {
  ...
  extractAndCompactStructure,
  compactRequirementsAndRationales,
  parseDocument,
  formatResult
};
...
if (require.main === module) {
  ...
  // Test dello script
  console.log('=== TEST DEL PARSER SEMPLICE ===');
  const result = extractStructureWithLines(testData);
  console.log(JSON.stringify(result, null, 2));
  
  console.log('\n=== TEST DEL PARSER COMPATTATO ===');
  const compactResult = extractAndCompactStructure(testData);
